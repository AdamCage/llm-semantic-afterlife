# Stage 5 report — distinguishable locks; twins collapse to the control

**Status.** Computations finished 2026-09-06. Overall verdict: **PASS**.
Awaiting scientific review. Do not merge until signed off.

On `or-qwen3-8b` under P1 `raw_completion`, at the S4 lock
(`W = 4096`, T=0.3, 12 turnovers), the ten domain seeds of
`seed_bank_v1` occupy distinguishable late-window locks: the
last-band gap CI excludes 0 in both embedding spaces. The two
one-fact twin pairs do **not**: last-band Δ CIs include 0 in both
spaces, pooled and per family. A lock is not a semantic basin.
...
| F6 | Twin last-band Δ, families + pooled | **PASS** | [`twin_last_band.csv`](../../../artifacts/stage-5/occupancy/twin_last_band.csv). Every last-band CI includes 0 → **collapsed**, both families, both spaces, and pooled |
| F7 | F4–F6 per embedding | **PASS** | Both spaces in the occupancy tables. F4 separated in both; F6 collapsed in both |
...
is degenerate, so the seed still has a locked replicate.
...
n=4 trajectories per family. The intervals are wide by construction.
They include 0; that is the collapsed verdict, not a precision claim.
...
| Q4 | Each twin family's last-band Δ CI includes 0 | 0.55 | **Right.** waterloo and reactor collapsed in both spaces. The interesting wrong (divergent) did not happen |
| Q5 | waterloo and reactor agree on Q4 | 0.50 | **Right.** Both collapsed in a given space |
| Q6 | F4 / F6 last-band verdicts agree across spaces | 0.70 | **Right.** Separated / collapsed in both |
...
Q2 and Q4 are both right: domains stay apart, twins do not.
A one-fact flip does not occupy a distinguishable lock on this
process at this operating point.
...
3. **Twins collapse, including a negative waterloo Δ in qwen3-embed-8b.**
   The factual flip is smaller than same-seed stochastic distance.
...
- **n=2 per seed.** F5 is a count. Twin family CIs are four
  trajectories; they include 0 and do not decide a small positive Δ.
...
1. **Occupancy is distinguishable on domains and collapsed on twins.**
   S6 must not rewrite this as recovered semantic memory. The
   seed shapes the lock; a one-fact flip does not stay apart from
