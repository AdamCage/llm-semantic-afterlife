...
Generate is **not** authorised. CLI estimate **$1.06** (fill=1);
S4 T=0.3 fill 0.90 is **~$1.17**. YAML refuse **$8**.

| pass | status |
| --- | --- |
| S5.0 reuse S2.2 raw `W=4096` T=0.3 physics/surreal (4 traj) | cells exist; not yet joined |
| S5.1 `stage5_lock_occupancy.yaml` (24 traj) | waiting for generate-yes |
